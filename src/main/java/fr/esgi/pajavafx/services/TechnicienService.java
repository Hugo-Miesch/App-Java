...
public class TechnicienService {
    private static final String API_URL = "http://jaegers-ops.duckdns.org:54122/techniciens";
    private final HttpClient client = HttpClient.newHttpClient();
    private final Gson gson = new Gson();

    public List<Technicien> fetchAll() {
        try {
            HttpRequest request = HttpRequest.newBuilder().uri(URI.create(API_URL)).build();
            HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
            Type listType = new TypeToken<List<Technicien>>() {}.getType();
    ...
    public Technicien add(Technicien t) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    ...
                    .build();
            HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
            return gson.fromJson(response.body(), Technicien.class);
